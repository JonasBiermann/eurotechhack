...
- **Cost-savings computed from real sourced cost data.** The headline "how much you'd save" figures (`gross_savings`, `net_savings`, `monthly_savings`, `pct_income_freed`, `runway_years` in `backend/projections.py`) are computed from **real published statistics**, not invented numbers: per-city rent + cost-of-living for all 8 GBA cities **and** the HK baseline are taken like-for-like from **Numbeo (Jun 2026, 1 CNY = 1.157 HKD)**; the forfeited public-housing subsidy (HK$9,187/mo) is the HA 2019/20 PRH implicit rent subsidy; care figures use the SWD GDRCS basis. These carry a dedicated `sourced` provenance badge (`config.PROVENANCE`). **Honest caveats:** it is a *dated manual snapshot* of those statistics (not a live API at runtime), and care-home/livability proxies are still modeled — see §3.
...
|---|---|---|---|
| GBA destination **cost** figures (rent, cost-of-living) | `backend/data/destinations.json`, `backend/seed_gba.py`, `config.py` | **Now real `sourced` data** — per-city rent + COL for all 8 cities and the HK baseline taken like-for-like from Numbeo (Jun 2026, 1 CNY=1.157 HKD). Caveat: a **dated manual snapshot**, not a live feed | Refresh from a live cost-of-living feed; already badged `sourced` (not `modeled`) in the provenance map |
| GBA destination **livability proxies** (care-home price, cantonese/air/green/amenity 0–1 scores) | `backend/data/destinations.json`, `backend/seed_gba.py` | Still **modeled** — the HK open datasets don't cover mainland livability; proxies tuned so the ranking visibly reacts in the demo | Pull real per-city livability/care-home data; values badged `modeled` in the provenance map |
...
| Projection numbers (GBA time-to-care, return burden, projected wellbeing) | `backend/projections.py`, constants in `backend/config.py` | Still-modeled formulas with hardcoded constants (return-trip model, GBA wait of 1 or 4 weeks). **The cost-savings side is now `sourced`** (Numbeo rent/COL + income-capped HK baseline); HK SOP wait is `real`; the GBA care-wait side stays **modeled** | Calibrate the care-wait / wellbeing side against real cross-border-care data |
| Benefit values in the ledger | `backend/benefits.py`, `config.py` | Allowance amounts (OAA/OALA, EHCV) are **real**; the public-housing implicit subsidy is now `sourced` (HK$9,187/mo, HA 2019/20 PRH figure, and `netted` against the income-capped baseline to avoid double-counting); community-services value (HK$800) remains a **modeled** estimate | Source the per-household community-services value |
...
| Numbeo cost-of-living (8 GBA cities + HK baseline) | Match score & the cost-savings calculation | **Real `sourced` snapshot** — Numbeo figures (Jun 2026, 1 CNY=1.157 HKD) manually captured into `destinations.json` / `config.py`, dated; not fetched live at runtime | None |
| GBA destination livability proxies (care-home, cantonese/air/green/amenity) | Match score | **Mocked/modeled** seed data | n/a |
...
- **GBA cost data is real and sourced; livability proxies are still illustrative.** Per-city rent + cost-of-living (and the HK baseline) are sourced from Numbeo (Jun 2026) — a dated manual snapshot, not a live feed — so the cost-savings figures are computed on real numbers. The care-home/cantonese/air/green/amenity 0–1 proxies remain modeled and tuned for the demo and should be replaced with real per-city data before that part of the ranking is trusted.
